...
class Pattern:
    def __init__(self, pt_size):
        ...
        self.delay = 110       # initial time delay
        self.size = self.width, self.height = 580, 1000  # the size of background
        self.pt_size = 0.7*pt_size  # the size of pattern
        self.step = max(1, self.pt_size // 20)
        # the step length of pattern's moving
        #self.step = 1
    ...
    def draw_text(self, text, position, delay = False):
        self.screen.fill(self.bg)
        pg.display.set_caption('Auto LAMP')
        font = pg.font.Font(None, 140)
        (x, y) = position
    ...
    def move_up(self):
        ...
        self.screen.fill(self.bg)
        # pg.draw.rect(screen, pttern, (x - pt_size, y - 2 * pt_size, 2 * pt_size, 3 * pt_size), 0)
        pg.draw.rect(self.screen, self.pattern, (0, 0, x - self.pt_size * 1.5, y + self.pt_size ), 0)
        pg.draw.rect(self.screen, self.pattern,
                     (x + self.pt_size * 1.5, y, self.width - x - self.pt_size * 1.5, self.height - y ), 0)
        pg.draw.rect(self.screen, self.pattern,
                     (x - self.pt_size * 1.5, y, self.pt_size * 3, self.pt_size), 0)
        pg.display.flip()
    ...
    def move_down(self):
        ...
        self.screen.fill(self.bg)
        # pg.draw.rect(screen, pttern, (x - pt_size, y - 2 * pt_size, 2 * pt_size, 3 * pt_size), 0)
        pg.draw.rect(self.screen, self.pattern, (0, 0, x - self.pt_size * 1.5, y), 0)
        pg.draw.rect(self.screen, self.pattern,
                     (x + self.pt_size * 1.5, y - self.pt_size, self.width - x - self.pt_size * 1.5, self.height - y + self.pt_size ), 0)
        pg.draw.rect(self.screen, self.pattern,
                     (x - self.pt_size * 1.5, y - self.pt_size, self.pt_size * 3, self.pt_size), 0)
        pg.display.flip()
