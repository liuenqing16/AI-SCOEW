...
class Map():
    ...
    def createBlock(self, block_center, radius, path_width):
        x, y = block_center
        block_x.append(x)
        block_y.append(y)
        block_radius.append(radius)
        for m in range(x - radius - path_width, x + radius + path_width):
            for n in range(y - radius - path_width, y + radius + path_width):
                self.map[n][m] = 1
...
def showPath(width, height, path_width, dest):
    ...
    for i in range(len_path):
        ...
        for j in range(len(best_path)):
            ...
            pg.draw.rect(screen, LIGHTRED, (path_x - path_width, path_y - path_width, path_width * 2, path_width * 2), 0)

        pg.draw.rect(screen, RED, (dest_x - path_width, dest_y - path_width, path_width * 2, path_width * 2), 0)

        # pg.draw.circle(screen, BLUE, center, path_width)
    ...
    while True:
        ...
        pg.draw.circle(screen, BLUE, (dest_x, dest_y), path_width)
...
def finding():
    ...
    map.createBlock(BLOCK_NUM, radius)
    map.showMap()
