import sys
sys.path.append("D:\install\pythonProject\DLAM-feedback\\thorlabs_camera")
import os
...
class Camera():
    def __init__(self):
        ...
        self.path = r'D:/install/pythonProject\DLAM-feedback/temp_image/'
            ...
            with TLCameraSDK.open_camera(camera_list[1]) as TLCamera:   # open camera
                ...
                '''with tl_mono_to_color_processor.MonoToColorProcessorSDK() as MonoToColorProcessorSDK:
                    with MonoToColorProcessorSDK.create_mono_to_color_processor(sensor_type, color_filter, color_correction, white_balance, bit_depth) as MonoToColorProcessor:
                        ...
                        return img_name'''
                with tl_mono_to_color_processor.MonoToColorProcessorSDK() as MonoToColorProcessorSDK:
                    with MonoToColorProcessorSDK.create_mono_to_color_processor(sensor_type, color_filter,
                                                                                color_correction, white_balance,
                                                                                bit_depth) as MonoToColorProcessor:
                        image_array = MonoToColorProcessor.transform_to_24(image_buffer, self.roi_size,
                                                                           self.roi_size)
                        image_array = np.reshape(image_array, (self.roi_size, self.roi_size, 3))
                        img = Image.fromarray(image_array)
                        #img_name = self.path + str(int(time.time())) + '.jpg'
